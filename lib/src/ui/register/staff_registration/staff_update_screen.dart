...
class StaffUpdateScreen extends StatefulWidget {
  ...
  StaffUpdateScreen({super.key, required this.staff});

  @override
  State<StaffUpdateScreen> createState() => _StaffUpdateScreenState();
}
...
class _StaffUpdateScreenState extends State<StaffUpdateScreen> {
                              ...
                              BlocConsumer<ImagePickBloc, ImagePickState>(
                                ...
                                builder: (context, state) {
                                  if (state is ImagePickSuccess) {
                                    ...
                                    StudentData.resetImage();
                                    return (widget.staff.image) != ""
                                        ? CircleAvatar(
                                            radius: 100,
                                            backgroundImage: NetworkImage(
                                                "${ApisEndpoints.imagesPathStaff}${widget.staff.image}"),
                                          )
                                        : CircleAvatar(
                                            radius: 100,
                                            backgroundImage:
                                    ...
                                    onDoneFuction: () async {
                                      if (nameController.text.isEmpty) {
                                        ...
                                        var map = {
                                          'id': '${widget.staff.id}',
                                          'college_id':
                                              '${widget.staff.collegeId}',
                                          'user_id': '${widget.staff.userId}',
                                          'session': "${widget.staff.session}",
  ...
  void initController() {
    ...
    mobileController.text = widget.staff.mobileNo;
    fatherController.text = widget.staff.father;
    dobController.text = DateFormat('yyyy-MM-dd').format(widget.staff.dob.toLocal()).toString();
    pincodeController.text = widget.staff.pinCode;
    stateController.text = widget.staff.state;
    districtController.text = widget.staff.district;
    tehsilController.text = widget.staff.tehsil;
    villMohallaController.text = widget.staff.villageMohalla;
  }
}
//
